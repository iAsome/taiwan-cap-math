...
for (const file of ["civics-data.js", "quiz-taxonomy.js", "questions.js", "analysis-data.js"]) {
  const code = fs.readFileSync(path.join(root, file), "utf8");
  vm.runInContext(code, sandbox, { filename: file });
}

const { CIVICS_DATA, EXAM_ENGINE, CIVICS_ANALYSIS } = sandbox.window;
const errors = [];
...
const analysisObj = CIVICS_ANALYSIS;
const analysisName = "CIVICS_ANALYSIS";
...
if (errors.length) {
  ...
  const years = Object.keys(CIVICS_ANALYSIS.primaryUnits).sort();
  const totalArchiveQ = years.reduce((s, y) => s + CIVICS_ANALYSIS.primaryUnits[y].length, 0);
