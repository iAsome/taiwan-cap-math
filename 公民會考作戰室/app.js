(() => {
  const { units, domains, strategies, archives, sourcePolicy, officialSources, publisherSources, tipAudits, groups } = window.CIVICS_DATA;
  const capAnalysis = window.CIVICS_ANALYSIS;
  ...
  const abilityLabel = { knowledge: "知識理解", comprehension: "統整判讀", inquiry: "公民實踐" };
  const viewNames = { home: "學習總覽", exam: "全範圍模擬考", quiz: "單元小考題庫", papers: "我的考卷", handbook: "國中公民全冊講義", atlas: "題型與技巧地圖", analysis: "近十年逐題分析", sources: "資料與技巧審核", archive: "近十年考卷館" , paper: "官方考卷" };
  let toastTimer;
  ...
  const state = {
    ...
    tipSearch: "",
    tipVerdict: "all",
    completed: new Set(JSON.parse(localStorage.getItem("capCivics.completed") || "[]")),
    exam: null,
    answers: [],
    submitted: false,
    seconds: 2400,
  ...
  function uniqueQuizAssessment(quizId) {
    const used = readJson(`capCivics.quizSignatures.${quizId}`, []);
    for (let attempt = 0; attempt < 80; attempt++) {
      ...
      if (!used.includes(signature)) {
        localStorage.setItem(`capCivics.quizSignatures.${quizId}`, JSON.stringify([signature, ...used].slice(0, 300)));
        return assessment;
      }
    }
  ...
  function paperHistory() { return readJson("capCivics.paperHistory", []); }

  function savePaperRecord(record) {
    try { localStorage.setItem("capCivics.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 40))); }
    catch {
      try { localStorage.setItem("capCivics.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 15))); }
  ...
  function renderAtlas() {
    $("#atlasContent").innerHTML = `
      ...
      <section class="strategy-section"><p class="eyebrow">PATTERN → TOOL</p><h2>常見公民題型：看到什麼，就啟動什麼判斷方式</h2>
  ...
  function renderArchive() {
    $("#archiveGrid").innerHTML = archives.map((item, index) => `
      <article class="archive-card ${index === 0 ? "latest" : ""}">
        ...
        <div class="archive-body"><h3>${item.year} 年社會科公民題</h3><p>官方題本 PDF｜可預覽與下載</p><small class="archive-scope-note">整份社會科官方題本（非僅公民篩選版）</small>
  ...
  function renderAnalysis() {
    ...
    const total = years.reduce((sum, year) => sum + (primary[year]?.length || 0), 0);
    $("#analysisStats").innerHTML = [
      [years.length || "0", "屆官方社會科題本"], [total, "逐題完成主概念編碼"], [years.length ? Math.round(total / years.length * 10) / 10 : 0, "平均每屆公民題數"]
    ...
    const abilityMax = Math.max(1, ...Object.values(abilityTotals));
    ...
    $("#officialStructureTable").innerHTML = `<table class="mini-table"><thead><tr><th>年度</th><th>公民題數</th><th>知識理解</th><th>統整判讀</th><th>公民實踐</th></tr></thead><tbody>${years.map(year => `<tr><td>${year}</td><td>${official[year].count}</td><td>${official[year].abilities.knowledge}</td><td>${official[year].abilities.comprehension}</td><td>${official[year].abilities.inquiry}</td></tr>`).join("") || `<tr><td colspan="5" class="audit-empty">尚無凍結分析資料。</td></tr>`}</tbody></table>`;
    ...
    $("#domainDistribution").innerHTML = domainOrder.map(domain => `<article class="domain-stat"><span>${domain}</span><strong>${domainCounts[domain] || 0}</strong><small>${total ? ((domainCounts[domain] || 0) / total * 100).toFixed(1) : "0.0"}%｜十年合計</small></article>`).join("");
    ...
    $("#yearLedger").innerHTML = years.length ? years.slice().reverse().map((year, index) => {
      const info = official[year];
      return `<details ${index === 0 ? "open" : ""}><summary><strong>${year}</strong><span>${info.count} 題公民｜社會科全卷分配估算</span><b>展開逐題編碼 ＋</b></summary><div class="ledger-body"><div class="ledger-ability"><span>知識理解 ${info.abilities.knowledge}</span><span>統整判讀 ${info.abilities.comprehension}</span><span>公民實踐 ${info.abilities.inquiry}</span></div><div class="ledger-questions">${primary[year].map((unitId, i) => `<article class="ledger-item"><b>${i + 1}</b><span>${esc(units.find(u => u.id === unitId)?.title || "跨單元")}<small>${esc(capAnalysis.domainByUnit[unitId])}</small></span></article>`).join("")}</div></div></details>`;
    }).join("") : `<div class="paper-history-empty"><h2>凍結分析資料。</h2><p>本頁年度帳本依凍結的 analysis-data.js 顯示；官方題本請至考卷館下載 PDF。</p></div>`;
  }
  ...
  function capSummary(item, compact = false) {
    ...
    return `<div class="quiz-cap-tags"><strong>會考標註 ${items.length} 題</strong><small>106–115 年社會科公民主概念回查</small><div class="quiz-cap-preview">${preview || "<span>近十年凍結分析資料；仍依課綱出題</span>"}</div></div>`;
  }
      ...
      return `<details class="unit-book-ledger" open><summary><strong>${esc(g.name)}</strong><span>${items.length} 個單元｜會考主概念標註 ${count} 題</span><b>展開／收合</b></summary>
        <div class="unit-chapter-list">${items.map(item => {
          ...
          return `<article class="unit-chapter-item">
            ...
            <div class="unit-cap-list">${capItems.map(cap => `<i title="${esc(cap.unitTitle)}">${esc(cap.label)}</i>`).join("") || "<i>近十年凍結分析資料；仍依課綱出題</i>"}</div>
  ...
  function configureExamHeader() {
    ...
    $("#examDescription").textContent = isQuiz
      ...
      : isArchive
      ? `依 ${state.exam.year} 年官方社會科題本中的公民題目製作的可作答電子試卷；計時結束後仍可繼續作答。`
      : "30 題四選一，共 40 分鐘。涵蓋 20 個公民單元，法律與生活、政治與民主治理兩大領域加抽比重；本規格為本站自訂比例，非官方獨立考科規格。";
  ...
  function launchAssessment(assessment) {
    assessment = window.TEXT_ONLY_POLICY?.prepareTextOnlyExam?.(assessment, "civics") || assessment;
  ...
  function beginExam() {
    ...
    assessment.title = "會考公民模擬考";
    assessment.minutes = 40;
    launchAssessment(assessment);
    localStorage.setItem("capCivics.lastSeed", seed);
  }
  ...
  function submitExam() {
    ...
    const resultNote = isQuiz ? `本小考只計入「${esc(state.exam.title)}」的範圍，原始答對數不等同學校定期評量成績。`
      : isArchive ? `本卷依 ${state.exam.year} 年官方公布題本中的公民題目重製，供練習使用；正式成績、等級與官方原始題本以會考官方網站為準。`
      : "本結果是練習用原始答對數，不等同官方等級；本模考規格為本站自訂，非官方獨立公民考科規格。";
  ...
  function bindStaticEvents() {
    ...
    $("#themeButton").addEventListener("click", () => {
      ...
      localStorage.setItem("capCivics.dark", dark ? "1" : "0");
  ...
  function init() {
    const lastSeed = localStorage.getItem("capCivics.lastSeed");
    ...
    if (localStorage.getItem("capCivics.dark") === "1") { document.body.classList.add("dark"); $("#themeButton").textContent = "日"; }
    bindStaticEvents();
