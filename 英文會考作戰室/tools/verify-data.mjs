...
for (const file of ["english-data.js", "quiz-taxonomy.js", "questions.js", "analysis-data.js", "archive-exams.js"]) {
  const code = fs.readFileSync(path.join(root, file), "utf8");
  vm.runInContext(code, sandbox, { filename: file });
}

const { ENGLISH_DATA, QUIZ_TAXONOMY, EXAM_ENGINE, ENGLISH_ANALYSIS, ARCHIVE_EXAMS } = sandbox.window;
const errors = [];
...
Object.entries(ARCHIVE_EXAMS).forEach(([year, exam]) => {
  exam.questions.forEach((q, i) => {
    check(unitIds.includes(q.unitId), `archive-exams ${year} 第 ${i + 1} 題 unitId=${q.unitId} 不存在於 ENGLISH_DATA`);
...
if (ENGLISH_ANALYSIS) {
  ...
  Object.keys(ARCHIVE_EXAMS).forEach(year => check(!!ENGLISH_ANALYSIS.primaryUnits[year], `archive-exams 有 ${year} 年，但 analysis-data primaryUnits 沒有登記`));
} else {
  check(Object.keys(ARCHIVE_EXAMS).length === 0, "archive-exams 已有資料，但 analysis-data 尚未建立（ENGLISH_ANALYSIS 為 null）");
}
...
if (errors.length) {
  ...
  console.log(`全部檢查通過：20 個單元、${EXAM_ENGINE.quizCatalog.length} 份小考、模考 30 題、考卷典藏 ${years.length} 年（${years.join("、")}）共 ${totalArchiveQ} 題。`);
}
