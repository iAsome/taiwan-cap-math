...
window.ENGLISH_DATA = (() => {
  ...
  const units = [
...

    {
      ...
      clarify: "會考字彙題很少考『詞意完全生僻』的單字，更常考『兩個意思相近的字，哪一個才符合上下文語氣或固定搭配』（例如 borrow 與 lend 方向相反、arrive 與 reach 的介系詞用法不同），因此準備重點應放在『易混淆的近義字組』，而不是漫無目的擴充生字量。",
      ...
      derivation: "英語單字的意義往往必須依賴使用情境與固定搭配才能精準判斷，因此把單字按主題與情境分類記憶，能同時記住『詞意＋典型用法』，比孤立背誦單字表更接近實際考題的判斷方式；而易混淆字組的題目設計，本質上是在測驗考生是否只背了『中文翻譯』還是真正掌握了『使用情境與方向性』（如借入或借出、帶來或帶走），這也是準備字彙題時最需要花時間釐清的部分。",
    ...
    },
    {
      ...
      core: "閱讀測驗允許在文中出現少量超出基本字彙表的生字（通常會加註中文），但考生仍需要具備『從構詞與上下文猜測詞意』的能力：常見字首（un-/dis- 表否定、re- 表再次、pre- 表事先）與字尾（-ful/-less 表有/無某特質、-tion/-ment 表名詞化、-able 表可以...的）能提供詞性與大致方向的線索；再搭配上下文的因果、對比、舉例關係，可以推敲出生字的合理意思，不需要百分之百查得到精確中文翻譯。",
      clarify: "猜詞意的目的是『讀懂句子在說什麼、能不能答題』，不是『翻譯出精確的中文字典釋義』；考生常見的錯誤心態是卡在一個生字上反覆糾結，反而忽略了利用前後句提供的線索（例如同位語、舉例、對比連接詞 but/however）就能直接推敲出大致方向。",
      ...
      tips: ["un-/in-/dis- 開頭的字，先假設是原本詞意的『相反』", "-tion/-ment/-ness 結尾通常是名詞，可以幫助判斷這個字在句子裡的角色", "however、but、although 後面常常出現與前面意思相反的線索", "遇到生字不要停頓太久，先猜大致方向、把整句讀完再回頭確認"],
