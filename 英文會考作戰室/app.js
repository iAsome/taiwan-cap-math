...
const OFFICIAL_EXAMS_URL = "https://cap.rcpet.edu.tw/examination.html";
  ...
  function renderArchive() {
    $("#archiveGrid").innerHTML = archives.map((item, index) => `
      <article class="archive-card ${index === 0 ? "latest" : ""}">
        ...
        <div class="archive-body"><h3>${item.year} 年國中教育會考英語（閱讀）</h3><p>官方題本 PDF｜可預覽與下載</p>
  ...
  function renderAnalysis() {
    if (!capAnalysis) {
      ...
      ["#abilityBars", "#officialStructureTable", "#domainDistribution", "#conceptMatrixHead", "#conceptMatrixBody", "#unitChapterLedger", "#yearLedger"].forEach(sel => { const el = $(sel); if (el) el.innerHTML = ""; });
      return;
    }
