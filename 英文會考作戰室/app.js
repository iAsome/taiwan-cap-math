(() => {
  ...
  const viewNames = { home: "學習總覽", exam: "全範圍模擬考", quiz: "單元小考題庫", papers: "我的考卷", handbook: "國中英語全冊講義", atlas: "題型與技巧地圖", analysis: "近十年逐題分析", sources: "資料與技巧審核", archive: "近十年考卷館" };
  let toastTimer;
  ...
  const state = {
    ...
    tipSearch: "",
    tipVerdict: "all",
    completed: new Set(JSON.parse(localStorage.getItem("capEnglish.completed") || "[]")),
    exam: null,
    answers: [],
    submitted: false,
    seconds: 2400,
  ...
  function uniqueQuizAssessment(quizId) {
    const used = readJson(`capEnglish.quizSignatures.${quizId}`, []);
    for (let attempt = 0; attempt < 80; attempt++) {
      ...
      if (!used.includes(signature)) {
        localStorage.setItem(`capEnglish.quizSignatures.${quizId}`, JSON.stringify([signature, ...used].slice(0, 300)));
        return assessment;
      }
    }
  ...
  function paperHistory() { return readJson("capEnglish.paperHistory", []); }

  function savePaperRecord(record) {
    try { localStorage.setItem("capEnglish.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 40))); }
    catch {
      try { localStorage.setItem("capEnglish.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 15))); }
  ...
  function renderHandbook() {
    ...
    $("[data-complete]").addEventListener("click", () => {
      ...
      localStorage.setItem("capEnglish.completed", JSON.stringify([...state.completed]));
      updateLearningProgress();
      renderHandbook();
  ...
  function renderAtlas() {
    $("#atlasContent").innerHTML = `
      ...
      <section class="strategy-section"><p class="eyebrow">PATTERN → TOOL</p><h2>常見英文題型：看到什麼，就啟動什麼判斷方式</h2>
  ...
  function renderArchive() {
    $("#archiveGrid").innerHTML = archives.map((item, index) => {
      ...
      <article class="archive-card ${index === 0 ? "latest" : ""}">
        ...
        <div class="archive-body"><h3>${item.year} 年國中教育會考英語（閱讀）試題</h3><p>${data ? `${count} 題選擇｜依官方英語閱讀題本製作，可作答並看逐題詳解` : "電子試卷建置中，敬請期待"}</p>
  ...
  function beginArchiveExam(year) {
    ...
    launchAssessment({
      ...
      id: `ARCHIVE-${year}`,
      year,
      title: `${year} 年國中教育會考英語（閱讀）試題`,
      minutes: data.minutes || 60,
      officialCodes: `official-${year}-english-reading`,
  ...
  function renderAnalysis() {
    if (!capAnalysis) {
      $("#analysisStats").innerHTML = `<div class="research-limit" style="grid-template-columns:1fr"><p>尚未完成 106–115 年官方英語閱讀題本逐題核對主概念，本頁統計尚未建置，避免呈現未經查證的數字。</p></div>`;
    ...
    $("#analysisStats").innerHTML = [
      ["10", "屆官方英語閱讀題本"], [total, "逐題完成主概念編碼"], [Math.round(total / years.length * 10) / 10, "平均每屆閱讀題數"]
    ...
    $("#officialStructureTable").innerHTML = `<table class="mini-table"><thead><tr><th>年度</th><th>閱讀題數</th><th>字彙文法</th><th>篇章理解</th><th>推論應用</th></tr></thead><tbody>${years.map(year => `<tr><td>${year}</td><td>${official[year].count}</td><td>${official[year].abilities.knowledge}</td><td>${official[year].abilities.comprehension}</td><td>${official[year].abilities.inquiry}</td></tr>`).join("")}</tbody></table>`;
    ...
    $("#yearLedger").innerHTML = years.slice().reverse().map((year, index) => {
      ...
      return `<details ${index === 0 ? "open" : ""}><summary><strong>${year}</strong><span>${info.count} 題閱讀｜英語科官方閱讀試題本</span><b>展開逐題編碼 ＋</b></summary><div class="ledger-body"><div class="ledger-ability"><span>字彙文法 ${info.abilities.knowledge}</span><span>篇章理解 ${info.abilities.comprehension}</span><span>推論應用 ${info.abilities.inquiry}</span></div><div class="ledger-questions">${primary[year].map((unitId, i) => `<article class="ledger-item"><b>${i + 1}</b><span>${esc(units.find(u => u.id === unitId)?.title || "跨單元")}<small>${esc(capAnalysis.domainByUnit[unitId])}</small></span></article>`).join("")}</div></div></details>`;
    }).join("");
  }
  ...
  function capSummary(item, compact = false) {
    ...
    return `<div class="quiz-cap-tags"><strong>會考標註 ${items.length} 題</strong><small>106–115 年英語閱讀主概念回查</small><div class="quiz-cap-preview">${preview || "<span>近十年未列為主概念；仍依課綱出題</span>"}</div></div>`;
  }
  ...
  function quizCard(item) {
    ...
    return `<article class="quiz-card ${isReview ? "total" : "chapter"}">
      ...
      <p>${isReview ? "彙整同領域所有單元的小節題目，供領域總複習使用；交卷後逐題顯示文法規則、詳解與易錯點。" : "四選一、即時計分、逐題詳解；題目只從此單元生成。"}</p>
  ...
  function configureExamHeader() {
    ...
    $("#examTitle").textContent = isQuiz || isArchive ? state.exam.title : "會考英文文法模擬考";
  ...
  function renderExam() {
    ...
    const cover = isQuiz ? `
      ...
      <header class="paper-cover"><div><p class="eyebrow">官方題本重現 · ${esc(state.exam.id)}</p><h2>${state.exam.year} 年國中教育會考英語（閱讀）試題</h2><p>${state.exam.questions.length} 題選擇｜依官方公布的英語閱讀題本製作為可作答電子試卷｜計時結束仍可繼續作答</p></div><div class="paper-stamp">${state.exam.year}<br>官方題本</div></header>
      ...
      <header class="paper-cover"><div><p class="eyebrow">自訂比例 · 涵蓋 20 單元 · ${esc(state.exam.id)}</p><h2>會考英文文法模擬考</h2><p>30 題選擇｜40 分鐘｜閱讀技巧與核心字彙兩大領域加抽比重（本站自訂文法字彙隨機練習，非官方閱讀測驗原始規格）</p></div><div class="paper-stamp">英文<br>自訂規格</div></header>
  ...
  function bindStaticEvents() {
    ...
    $("#themeButton").addEventListener("click", () => {
      ...
      localStorage.setItem("capEnglish.dark", dark ? "1" : "0");
  ...
  function init() {
    const lastSeed = localStorage.getItem("capEnglish.lastSeed");
    ...
    if (localStorage.getItem("capEnglish.dark") === "1") { document.body.classList.add("dark"); $("#themeButton").textContent = "日"; }
    bindStaticEvents();
