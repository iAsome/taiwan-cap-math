...
const SKILLS = [
  {
    ...
    questions: [
      ...
      ["After cooking, the soup smells ___.", ["well", "good", "goodness", "betterly"], 1, "smells 在此連接 soup 與其狀態，後接形容詞 good。", ["well 常作副詞；本句不是說聞的動作做得好。", "名詞 goodness 不能直接作狀態補語。", "betterly 不是正確英文字形。"], "linking-verb-cloze", "distinguish-state-from-manner", "use-adverb-after-linking-verb"],
  ...
  },
  {
    ...
    questions: [
      ["The bridge road was closed, ___ we took another way.", ["because", "so", "but", "or"], 1, "道路封閉是原因，改走別路是結果，so 引出結果。", ["because 後應接原因，但此處後句是結果。", "but 需要預期落差，本句是自然因果。", "or 表選擇，不能說明已採取的結果。"], "cause-result-cloze", "select-so-for-result", "reverse-because-and-so"],
