...
for (const file of ["history-data.js", "quiz-taxonomy.js", "questions.js", "analysis-data.js", "archive-exams.js"]) {
  const code = fs.readFileSync(path.join(root, file), "utf8");
  vm.runInContext(code, sandbox, { filename: file });
}

const { HISTORY_DATA, EXAM_ENGINE, HISTORY_ANALYSIS, ARCHIVE_EXAMS } = sandbox.window;
const errors = [];
...
Object.entries(ARCHIVE_EXAMS).forEach(([year, exam]) => {
  exam.questions.forEach((q, i) => {
    check(unitIds.includes(q.unitId), `archive-exams ${year} 第 ${i + 1} 題 unitId=${q.unitId} 不存在於 HISTORY_DATA`);
...
//    （目前 archive-exams / analysis-data 皆為空殼佔位，故以下迴圈在資料補齊前不會執行任何檢查）
Object.entries(HISTORY_ANALYSIS.primaryUnits).forEach(([year, unitSeq]) => {
  ...
  const info = HISTORY_ANALYSIS.officialByYear[year];
...
Object.keys(ARCHIVE_EXAMS).forEach(year => check(!!HISTORY_ANALYSIS.primaryUnits[year], `archive-exams 有 ${year} 年，但 analysis-data primaryUnits 沒有登記`));

// 5. domainByUnit 必須涵蓋所有單元 id
unitIds.forEach(id => check(!!HISTORY_ANALYSIS.domainByUnit[id], `analysis-data domainByUnit 缺少單元 ${id}`));
