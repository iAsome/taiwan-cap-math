// 歷史模擬考／小考出題引擎。
// 社會科（含歷史）會考全為四選一選擇題，沒有非選擇題，因此本引擎不需要 cr() helper。
...
window.EXAM_ENGINE = (() => {
  ...
  function withTextOnlyPolicy(question) {
    return typeof TEXT_ONLY_POLICY !== "undefined" ? TEXT_ONLY_POLICY.normalizeQuestion(question, "history") : question;
  }
  ...
  const reviewQuizzes = window.HISTORY_DATA.groups.map(g => ({
    id: `d${g.id}-all`, group: g.id, chapter: "總複習", title: `${groupNames[g.id]}．領域總複習`,
    unitIds: U.filter(u => u.grade === g.id).map(u => u.id), officialCodes: `HISTORY-${groupNames[g.id]}`, scope: "review",
  ...
  function generateQuiz(quizId, seedOverride) {
    ...
    return {
      ...
      unitIds: [...blueprint.unitIds], blueprint: "NAER-108-curriculum-history-scope", taxonomySource: blueprint.source, questions
    };
  }
  ...
  function generate(seed, level = 2) {
    ...
    // ponytail: 歷史在會考中不是獨立考科，沒有官方獨立歷史規格可比照，本站自訂 30 題／40 分鐘，
    // 涵蓋全部 20 單元，並在台灣史（領域1、2）多加抽 10 題，頁面上會清楚聲明此為自訂比例。
    const allUnitIds = U.map(u => u.id);
    const extraWeighted = [...U.filter(u => u.grade === 1).map(u => u.id), ...U.filter(u => u.grade === 2).map(u => u.id)];
    ...
    return { id: `HISTORY-${seed}-${level}`, seed: Number(seed), level, createdAt: new Date().toISOString(), blueprint: "self-defined-30q-40min-history-scope", questions };
  }
