(() => {
  ...
  const state = {
    ...
    tipSearch: "",
    tipVerdict: "all",
    completed: new Set(window.MATH_V2_STORAGE?.readCompleted?.() || JSON.parse(localStorage.getItem("capMath.humanRc.r1.completed") || "[]")),
    exam: null,
    answers: [],
    submitted: false,
    seconds: 4800,
  ...
  async function uniqueQuizAssessment(quizId) {
    const used = readJson(`capMath.humanRc.r1.quizSignatures.${quizId}`, []);
    for (let attempt = 0; attempt < 80; attempt++) {
      ...
      if (!used.includes(signature)) {
        localStorage.setItem(`capMath.humanRc.r1.quizSignatures.${quizId}`, JSON.stringify([signature, ...used].slice(0, 300)));
        return assessment;
      }
    }
  ...
  function paperHistory() { return readJson("capMath.humanRc.r1.paperHistory", []); }

  function savePaperRecord(record) {
    try { localStorage.setItem("capMath.humanRc.r1.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 40))); }
    catch {
      try { localStorage.setItem("capMath.humanRc.r1.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 15))); }
  ...
  function updatePaperRecord(recordId, patch) {
    ...
    try { localStorage.setItem("capMath.humanRc.r1.paperHistory", JSON.stringify(records.slice(0, 40))); }
...
const OFFICIAL_EXAMS_URL = "https://cap.rcpet.edu.tw/examination.html";
  ...
  function quizCard(item) {
    ...
    return `<article class="quiz-card ${item.term === "總複習" ? "total" : ""} ${isChapter ? "chapter" : ""}">
      ...
      <div class="quiz-start-row">
        <label class="quiz-seed-label">種子碼<input type="number" min="1" max="999999" placeholder="隨機" data-quiz-seed="${item.id}" value="${localStorage.getItem("capMath.humanRc.r1.lastQuizSeed") || ""}"></label>
  ...
  async function beginExam() {
    ...
    try {
      ...
      localStorage.setItem("capMath.humanRc.r1.lastSeed", seed);
    } catch (error) {
  ...
  async function beginQuiz(quizId, seedOverride) {
    ...
    try {
      ...
      if (seed) localStorage.setItem("capMath.humanRc.r1.lastQuizSeed", String(seed));
      launchAssessment(assessment);
    } catch (error) {
  ...
  function bindStaticEvents() {
    ...
    $("#themeButton").addEventListener("click", () => {
      ...
      localStorage.setItem("capMath.humanRc.r1.dark", dark ? "1" : "0");
  ...
  function init() {
    const lastSeed = localStorage.getItem("capMath.humanRc.r1.lastSeed");
    ...
    if (localStorage.getItem("capMath.humanRc.r1.dark") === "1") { document.body.classList.add("dark"); $("#themeButton").textContent = "日"; }
    bindStaticEvents();
    updateLearningProgress();
    ...
    if (requestedSeed) {
      ...
      localStorage.setItem("capMath.humanRc.r1.lastQuizSeed", requestedSeed);
    }
