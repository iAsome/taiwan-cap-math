window.MATH_LECTURE_V2_U05 = [
  ...
  },
  {
    ...
    "title": "二元一次方程式圖形",
    "concept": "二元一次方程式 ax+by=c 的圖形是直線。描點常用截距法：令 x=0 得 y 截距，令 y=0 得 x 截距。y=常數 是平行 x 軸的直線，x=常數 是平行 y 軸的直線。斜率 k 表示 x 每增 1 時 y 的變化量，k>0 向右上，k<0 向右下。錯選通常是把正負方向或軸向對調，應回到定義逐項核對。",
    "formula": "ax+by=c → 直線；y=kx+b 斜率 k",
    "stepGuide": [
      "確認是一次方程式。",
      "令 x=0 求 y 截距。",
      "令 y=0 求 x 截距。",
  ...
  },
  {
    ...
    "title": "點在直線上的判斷",
    "concept": "要判斷點 (x₀, y₀) 是否在直線 ax+by=c 上，把 x₀、y₀ 帶入方程式，若左邊等於右邊則在線上，否則不在。已知 x 求 y 或已知 y 求 x 也是同一原理。會考常給參數 a、b、k，先帶入再解一次方程式。干擾項常漏看另一坐標或符號，求值後要再對照題意。",
  ...
  },
  {
    ...
    "title": "截距值與截距點",
    "concept": "x 截距是直線與 x 軸交點的 x 坐標數值（令 y=0 求 x）；y 截距是與 y 軸交點的 y 坐標數值（令 x=0 求 y）。截距「值」是一個數；截距「點」是 (x,0) 或 (0,y) 的坐標。會考常混問截距值或交點坐標，要先讀清題目。這類題不能憑印象，須把條件寫成算式再比對選項。",
  ...
  },
  {
    ...
    "title": "兩直線交點",
    "concept": "兩直線交點坐標即聯立方程式的解。斜率相同且截距不同 → 平行，無交點；兩式完全相同 → 重合，無限多交點；斜率不同 → 唯一交點。水平線 y=常數 與鉛直線 x=常數 的交點為 (x常數, y常數)。易錯在只記口訣未套數字，帶入後才能排除其他選項。",
    "formula": "交點 = 聯立解；平行：斜率同且截距異",
    "stepGuide": [
      "化簡兩方程式。",
      "比斜率判平行或重合。",
      "非平行則聯立。",
