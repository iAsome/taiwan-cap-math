(() => {
  ...
  function renderArchive() {
    $("#archiveGrid").innerHTML = archives.map((item, index) => {
      ...
      const mc = data?.questions.filter(q => q.type === "mc").length || 0;
      const cr = data?.questions.filter(q => q.type === "cr").length || 0;
      return `
      <article class="archive-card ${index === 0 ? "latest" : ""}">
        ...
        <div class="archive-body"><h3>${item.year} 年國中教育會考</h3><p>${data ? `${mc} 題選擇＋${cr} 題非選｜依官方題本製作，可作答並看詳解` : "電子試卷建置中，敬請期待"}</p>
  ...
  function beginArchiveExam(year) {
    ...
    launchAssessment({
      ...
      id: `ARCHIVE-${year}`,
      year,
      title: `${year} 年國中教育會考數學科題本`,
      minutes: data.minutes || 80,
