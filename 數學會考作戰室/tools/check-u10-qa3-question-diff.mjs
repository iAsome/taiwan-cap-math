#!/usr/bin/env node
/** U10-QA3: assert editorial/literacy question diff matches manifest against fixed base. */
...
const BASE = "64f11dc8a521dab5f2b13e967d9ac6248a536797";
const tools = path.dirname(fileURLToPath(import.meta.url));
const root = path.dirname(tools);
const v2 = path.join(root, "v2");
const REPO = path.join(root, "..");
const MANIFEST = U10_QA3_REQUIRED_QUESTIONS;
const QA3_IDS = Object.keys(MANIFEST).sort();
...
export function assertU10Qa3QuestionDiff() {
  ...
  assert.equal(QA3_IDS.length, 8, "manifest question count");
  for (const id of QA3_IDS) {
  ...
  assert.equal(changedRecords, 8, "changed question records");
  assert.equal(changedFields, 24, "changed top-level fields");

  return { changedRecords, changedFields };
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const { changedRecords, changedFields } = assertU10Qa3QuestionDiff();
  console.log("check-u10-qa3-question-diff: OK");
