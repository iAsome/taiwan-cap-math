...
const BASE_COMMIT = "2ed73f093c23996a98bad2d13bf1091ede0be40d";
const ROOT = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const V2 = path.join(ROOT, "v2");
const REPO_ROOT = path.dirname(ROOT);

const MANIFEST_IDS = Object.keys(U08_QA2B1_REQUIRED_QUESTIONS).sort();
...
export function assertU08Qa2b1QuestionDiff() {
  assert.equal(MANIFEST_IDS.length, 10, "manifest question count");
  ...
  for (let i = 0; i < 144; i++) {
    ...
    if (JSON.stringify(base) !== JSON.stringify(current)) {
      changedRecords++;
      const manifest = U08_QA2B1_REQUIRED_QUESTIONS[base.questionId];
      assert.ok(manifest, `${base.questionId} missing from manifest`);
      changedFields += questionFieldDiff(base, current, manifest);
    } else {
      assert.ok(!U08_QA2B1_REQUIRED_QUESTIONS[base.questionId], `${base.questionId} changed without manifest`);
    }
  }

  assert.equal(changedRecords, 10, "changed question records");
  assert.equal(changedFields, 11, "changed top-level fields");
...
if (isMain) {
  const counts = assertU08Qa2b1QuestionDiff();
  console.log(
    `check-u08-qa2b1-question-diff: OK — ${counts.changedRecords} records, ${counts.changedFields} fields, ${counts.manifestQuestions} manifest questions`
  );
}
