#!/usr/bin/env node
/** U10-QA2B: assert square-difference question diff matches manifest against fixed base. */
...
const BASE = "9cf4a0073020ee2f970df02bfb67b14b71d85c01";
const tools = path.dirname(fileURLToPath(import.meta.url));
const root = path.dirname(tools);
const v2 = path.join(root, "v2");
const REPO = path.join(root, "..");
const MANIFEST = U10_QA2B_REQUIRED_QUESTIONS;
const QA2B_IDS = Object.keys(MANIFEST).sort();
...
export function assertU10Qa2bQuestionDiff() {
  ...
  assert.equal(QA2B_IDS.length, 9, "manifest question count");
  for (const id of QA2B_IDS) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const { changedRecords, changedFields } = assertU10Qa2bQuestionDiff();
  console.log("check-u10-qa2b-question-diff: OK");
