...
assert.equal(Object.keys(QA5B2A_REQUIRED_RECORDS).length, 9, "QA5B2A map must have 9 entries");
for (const [id, fields] of Object.entries(QA5B2A_REQUIRED_RECORDS)) {
  const q = questions.find(x => x.questionId === id);
  assert.ok(q, `${id} missing`);
  for (const [field, value] of Object.entries(fields)) {
