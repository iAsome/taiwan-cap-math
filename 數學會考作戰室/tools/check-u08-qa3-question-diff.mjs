...
const BASE_COMMIT = "7cf7ba2a253862003062ea47bfc3ab5bb0305550";
const ROOT = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const V2 = path.join(ROOT, "v2");
const REPO_ROOT = path.dirname(ROOT);

const MANIFEST_IDS = Object.keys(U08_QA3_REQUIRED_QUESTIONS).sort();
...
export function assertU08Qa3QuestionDiff() {
  assert.equal(MANIFEST_IDS.length, 13, "manifest question count");
  ...
  for (let i = 0; i < 144; i++) {
    ...
    if (JSON.stringify(base) !== JSON.stringify(current)) {
      changedRecords++;
      const manifest = U08_QA3_REQUIRED_QUESTIONS[base.questionId];
      assert.ok(manifest, `${base.questionId} missing from manifest`);
      changedFields += questionFieldDiff(base, current, manifest);
    } else {
      assert.ok(!U08_QA3_REQUIRED_QUESTIONS[base.questionId], `${base.questionId} changed without manifest`);
    }
  }

  assert.equal(changedRecords, 13, "changed question records");
...
if (isMain) {
  const counts = assertU08Qa3QuestionDiff();
  console.log(
    `check-u08-qa3-question-diff: OK — ${counts.changedRecords} records, ${counts.changedFields} fields, ${counts.manifestQuestions} manifest questions`
  );
}
