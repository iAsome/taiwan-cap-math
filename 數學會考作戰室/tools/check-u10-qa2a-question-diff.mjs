#!/usr/bin/env node
/** U10-QA2A: assert square-sum question diff matches manifest against fixed base. */
...
const BASE = "66a9ed6ff50e507c1891fb679c96ec531b746393";
const tools = path.dirname(fileURLToPath(import.meta.url));
const root = path.dirname(tools);
const v2 = path.join(root, "v2");
const REPO = path.join(root, "..");
const MANIFEST = U10_QA2A_REQUIRED_QUESTIONS;
const QA2A_IDS = Object.keys(MANIFEST).sort();
...
export function assertU10Qa2aQuestionDiff() {
  ...
  assert.equal(QA2A_IDS.length, 10, "manifest question count");
  for (const id of QA2A_IDS) {
  ...
  assert.equal(changedFields, 46, "changed top-level fields");

  return { changedRecords, changedFields };
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const { changedRecords, changedFields } = assertU10Qa2aQuestionDiff();
  console.log("check-u10-qa2a-question-diff: OK");
