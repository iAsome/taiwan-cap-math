...
const BASE_COMMIT = "90ece6caca2aacf754ea695d118c4072702d0312";
const ROOT = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const V2 = path.join(ROOT, "v2");
const REPO_ROOT = path.dirname(ROOT);

const MANIFEST_IDS = Object.keys(U08_QA2B2_REQUIRED_QUESTIONS).sort();
...
export function assertU08Qa2b2QuestionDiff() {
  assert.equal(MANIFEST_IDS.length, 17, "manifest question count");
  ...
  for (let i = 0; i < 144; i++) {
    ...
    if (JSON.stringify(base) !== JSON.stringify(current)) {
      changedRecords++;
      const manifest = U08_QA2B2_REQUIRED_QUESTIONS[base.questionId];
      assert.ok(manifest, `${base.questionId} missing from manifest`);
      changedFields += questionFieldDiff(base, current, manifest);
    } else {
      assert.ok(!U08_QA2B2_REQUIRED_QUESTIONS[base.questionId], `${base.questionId} changed without manifest`);
    }
  }

  assert.equal(changedRecords, 17, "changed question records");
  assert.equal(changedFields, 17, "changed top-level fields");
...
if (isMain) {
  const counts = assertU08Qa2b2QuestionDiff();
  console.log(
    `check-u08-qa2b2-question-diff: OK — ${counts.changedRecords} records, ${counts.changedFields} fields, ${counts.manifestQuestions} manifest questions`
  );
}
