...
const evidence = path.resolve(arg('--evidence') || arg('--output') || path.join(repo, '數學會考作戰室', 'tools', 'v2-human-content', 'global-repair', 'wave-1b', 'evidence'));
const outputFile = path.join(evidence, 'batch-verifier-matrix-wave-1b.json');
...
console.log(JSON.stringify({
  ...
  failed: results.filter(x => !x.pass).map(x => x.batch),
  outputFile
}, null, 2));
