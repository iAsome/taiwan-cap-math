...
export const trackedFiles = repo => runGit(repo, ["ls-files", "-z"]).split("\0").filter(Boolean).map(value => value.replaceAll("\\", "/"));
export const gitBlob = (repo, relative) => runGit(repo, ["hash-object", "--", relative]);
export const textFile = file => {
