...
const syllabusUnits=new Map(read(path.join(path.dirname(batchesRoot),'authoring','authoring-syllabus-lock-r2.json')).units.map(unit=>[unit.unitId,unit]));
...
for(const batch of dirs(batchesRoot)){
 ...
 for(const unitDir of dirs(path.join(batch,'units'))){const unitId=path.basename(unitDir);const syllabusUnit=syllabusUnits.get(unitId);if(!syllabusUnit)throw new Error(`Missing syllabus metadata for ${unitId}`);if(!units.has(unitId))units.set(unitId,{unitId,numericUnitId:Number(unitId.slice(1)),title:syllabusUnit.title,gradeBand:syllabusUnit.gradeBand,domain:syllabusUnit.domain,skills:[],sourceBatches:[]});const unit=units.get(unitId);if(!unit.sourceBatches.includes(batchName))unit.sourceBatches.push(batchName);
