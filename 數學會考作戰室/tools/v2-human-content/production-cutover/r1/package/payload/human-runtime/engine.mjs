...
export class HumanReleaseCandidateRuntime {
  ...
  adaptMc(question) {
    ...
    return {
      ...question, type: "mc", text: question.text ?? question.prompt ?? question.stem ?? "", v2UnitId: question.unitId, unitId: lock.numericId,
  ...
  adaptCr(question) {
    const lock = this.lockByUnit.get(question.unitId); const skill = this.skillById.get(question.skillId)?.skill;
    const answer = (question.fullCreditSolution || []).at(-1) || "依完整解法與評分規準作答。";
    return {
      ...
      explanation: (question.scoringNotes || []).join(" ") || "依策略適切性與表達完整性評分。",
  ...
  async generateUnitQuiz(unitId, seed) {
    ...
    return { kind: "quiz", id: `MATH-HUMAN-RC-QUIZ-${unitId}-${seed}`, quizId: `${unitId}-human-rc`, seed: Number(seed),
      ...
      officialCodes: `${questions.length} 項技能｜每技能 1 題`, unitIds: [unit.__lock.numericId], v2UnitIds: [unitId], blueprint: "human-rc-one-question-per-skill",
