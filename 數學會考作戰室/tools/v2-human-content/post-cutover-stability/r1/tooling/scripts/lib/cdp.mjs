...
export function findBrowser() {
  const c=[process.env.CHROME_PATH,process.env.EDGE_PATH,
  ...
  const found=c.find(x=>fs.existsSync(x)); if(!found) throw new Error("No Chrome/Edge found; set CHROME_PATH or EDGE_PATH"); return found;
}
...
class Session{
  ...
  waitFor(method,t=15000){return new Promise((r,j)=>{const x=setTimeout(()=>j(new Error(`Timed out ${method}`)),t);const h=p=>{clearTimeout(x);this.l.set(method,(this.l.get(method)||[]).filter(v=>v!==h));r(p)};this.on(method,h)})}
...
export async function launchBrowser(){
  ...
  const executable=findBrowser(),port=await freePort(),dir=fs.mkdtempSync(path.join(os.tmpdir(),"math-v2-stability-"));
