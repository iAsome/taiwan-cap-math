// Insert English reading gap questions 107–110 (part 1).
...
function once(find, repl, label) {
  if (repl.trim().slice(0, 40) && s.includes(repl.trim().slice(0, 40))) { console.log("skip", label); return; }
  const n = s.indexOf(find);
  if (n < 0) throw new Error("anchor missing: " + label + " :: " + find.slice(0, 50));
...
`const exam107 = [\n  withDiagram(mc(17, 1, "Look at the picture.  The girl is ____ the clothes.",
    ...
    "容易誤選 washing，因為題幹有 clothes，但圖示是打包而非清洗。"), imgDiagram("107", "q1.png", "第1題圖")),\n  mc(17, 1, "The ___ is so strong`, "107-q1");

once(`    "容易把『狗甩水比洗衣機厲害』這個比喻誤解成『洗衣機的靈感來自狗甩水』的因果關係，但文章其實只是在做效果上的比較，並未提及發明的由來。"),\n];\n\n// (32-34) Josh 與 Eric 討論「待用咖啡」計畫的對話`,
...
];\n\n// (32-34) Josh 與 Eric 討論「待用咖啡」計畫的對話`, "107-q31");
rmNote(107);
...
`const exam108 = [\n  withDiagram(mc(17, 1, "Look at the picture.  The mother duck is ____ the baby ducks across the street.",
    ...
    "容易誤選 carrying，但小鴨是自己走路，並非被抱著。"), imgDiagram("108", "q1.png", "第1題圖")),\n  mc(9, 1, "Ms. Liu usually gives`, "108-q1");

once(`    "不要以為 otherwise 表示「沒有想法」，其實它明確表達「不同的、相反的看法」。"),
...
`    "不要以為 otherwise 表示「沒有想法」，其實它明確表達「不同的、相反的看法」。"),
  ...
  mc(16, 3, "Four people read the news`, "108-q31");

once(`    "「不如去年同期」這句話是用來形容週末晚上的生意，容易被誤植到選項中形容平日生意。"),\n];\nattach(exam108Shopping`,
...
];\nattach(exam108Shopping`, "108-q35");
rmNote(108);
...
`const exam109 = [\n  withDiagram(mc(17, 1, "Look at the picture.  The man is holding one of the ____.",
    ...
    "看圖選字題先確認手持物品的形狀與用途。",
    "容易誤選 plates，但圖中物品明顯是杯子而非盤子。"), imgDiagram("109", "q1.png", "第1題圖")),\n  mc(17, 1, "My mom puts`, "109-q1");

once(`const exam109Chocolate = [\n  mc(16, 2, "What does them mean`,
`const exam109Chocolate = [\n  withDiagram(mc(15, 2, "From the reading, which picture best shows the \\"bain-marie\\" way of working with chocolate?",
  ...
  mc(16, 2, "What does them mean`, "109-q22");
rmNote(109);

once(`const exam110Singles = [\n  mc(1, 1, "Listen!  The baby`,
`const exam110Singles = [\n  withDiagram(mc(17, 1, "In the picture, the boy is ____ the old man.",
    ...
    "看圖選字題先辨識人物間的禮貌動作。",
    "容易誤選 smiling at，但圖示是鞠躬而非微笑。"), imgDiagram("110", "q1.png", "第1題圖")),\n  mc(1, 1, "Listen!  The baby`, "110-q1");

once(`], "110-set-17-18", passage18, "Joey 與 Lynn 談 Caldron 餐廳（第17題因需觀看照片選項，無法轉錄，故省略）");`,
...
], "110-set-17-18", passage18, "Joey 與 Lynn 談 Caldron 餐廳");`, "110-q17");

once(`], "110-set-19-21", passage1921, "Katie 的日記：關於綿羊 Woollie（第20題因需辨認照片選項，無法轉錄，故省略）");`,
...
], "110-set-19-21", passage1921, "Katie 的日記：關於綿羊 Woollie");`, "110-q20");
rmNote(110);
