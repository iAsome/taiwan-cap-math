// 自我檢查腳本：載入生物站的五個資料模組，檢查跨檔案的資料一致性。
// 用法：node tools/verify-data.mjs（在 生物會考作戰室/ 目錄下執行）
...
// 1. 單元 id 應為 1..15 連續整數
const unitIds = BIO_DATA.units.map(u => u.id);
check(unitIds.length === 15, `BIO_DATA.units 應有 15 個單元，實際 ${unitIds.length} 個`);
...
mock.questions.forEach((q, i) => {
  ...
  check(unitIds.includes(q.unitId), `模考第 ${i + 1} 題 unitId=${q.unitId} 不存在於 BIO_DATA`);
});
...
for (const blueprint of EXAM_ENGINE.quizCatalog) {
  ...
  blueprint.unitIds.forEach(id => check(unitIds.includes(id), `小考 ${blueprint.id} 引用的 unitId=${id} 不存在於 BIO_DATA`));
}
