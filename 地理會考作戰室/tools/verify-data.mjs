...
const { GEOGRAPHY_DATA, EXAM_ENGINE, GEOGRAPHY_ANALYSIS } = sandbox.window;
const errors = [];
...
const analysisObj = GEOGRAPHY_ANALYSIS;
const analysisName = "GEOGRAPHY_ANALYSIS";
...
if (errors.length) {
  ...
  const years = Object.keys(GEOGRAPHY_ANALYSIS.primaryUnits).sort();
  const totalArchiveQ = years.reduce((s, y) => s + GEOGRAPHY_ANALYSIS.primaryUnits[y].length, 0);
