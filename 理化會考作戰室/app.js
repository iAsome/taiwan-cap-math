...
const OFFICIAL_EXAMS_URL = "https://cap.rcpet.edu.tw/examination.html";
  ...
  function renderExam() {
    ...
    const cover = isQuiz ? `
      <header class="paper-cover"><div><p class="eyebrow">單元範圍</p><h2>${esc(state.exam.title)}</h2><p>${state.exam.questions.length} 題四選一｜${state.exam.minutes || 15} 分鐘｜每個列出單元至少覆蓋 1 題</p></div><div class="paper-stamp">國${gradeName(state.exam.grade)}<br>${esc(state.exam.term)}</div></header>
      ...
      <header class="paper-cover"><div><p class="eyebrow">官方題本重現</p><h2>${state.exam.year} 年國中教育會考自然科（理化部分）</h2><p>${mcCount} 題選擇｜依官方公布題目製作為可作答電子試卷｜計時結束仍可繼續作答</p></div><div class="paper-stamp">${state.exam.year}<br>官方題本</div></header>
