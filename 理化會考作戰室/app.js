(() => {
  ...
  const state = {
    ...
    tipSearch: "",
    tipVerdict: "all",
    completed: new Set(JSON.parse(localStorage.getItem("capScience.completed") || "[]")),
    exam: null,
    answers: [],
    submitted: false,
    seconds: 2400,
  ...
  function uniqueQuizAssessment(quizId) {
    const used = readJson(`capScience.quizSignatures.${quizId}`, []);
    for (let attempt = 0; attempt < 80; attempt++) {
      ...
      if (!used.includes(signature)) {
        localStorage.setItem(`capScience.quizSignatures.${quizId}`, JSON.stringify([signature, ...used].slice(0, 300)));
        return assessment;
      }
    }
  ...
  function paperHistory() { return readJson("capScience.paperHistory", []); }

  function savePaperRecord(record) {
    try { localStorage.setItem("capScience.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 40))); }
    catch {
      try { localStorage.setItem("capScience.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 15))); }
  ...
  function gradeName(grade) { return grade === 8 ? "二" : "三"; }
  ...
  function renderHandbook() {
    ...
    const grouped = [8, 9].map(grade => ({ grade, items: list.filter(u => u.grade === grade) })).filter(g => g.items.length);
    $("#unitIndex").innerHTML = grouped.map(group => `
      <div class="index-group"><strong>國中 ${group.grade === 8 ? "二年級" : "三年級"}</strong>
    ...
    $("#unitContent").innerHTML = `
      <article class="unit-card">
        <header class="unit-hero" data-number="${String(unit.id).padStart(2, "0")}">
          <div class="unit-meta"><span>國${gradeName(unit.grade)}</span><span>${esc(unit.domain)}</span><span>單元編號 ${String(unit.id).padStart(2, "0")}</span></div>
    ...
    $("[data-complete]").addEventListener("click", () => {
      ...
      localStorage.setItem("capScience.completed", JSON.stringify([...state.completed]));
      updateLearningProgress();
      renderHandbook();
  ...
  function renderAtlas() {
    $("#atlasContent").innerHTML = `
      ...
      <section class="strategy-section"><p class="eyebrow">PATTERN → TOOL</p><h2>常見題型：看到什麼，就啟動什麼</h2>
  ...
  function renderArchive() {
    $("#archiveGrid").innerHTML = archives.map((item, index) => {
      ...
      const mc = data?.questions.length || 0;
      return `
      <article class="archive-card ${index === 0 ? "latest" : ""}">
        ...
        <div class="archive-body"><h3>${item.year} 年國中教育會考自然科</h3><p>${data ? `${mc} 題選擇題（理化部分）｜可作答並看詳解` : "本站尚未取得官方 PDF 逐題轉錄，可點下方連結查看官方原始題本"}</p>
          <div class="archive-actions"><button class="primary" data-archive-year="${item.year}" ${data ? "" : "disabled"}>${data ? "進入考試 →" : "建置中"}</button><a class="secondary link-button" href="${item.page}" target="_blank" rel="noopener">官方原始題本 ↗</a></div>
        </div>
      </article>`;
    }).join("");
  ...
  function beginArchiveExam(year) {
    ...
    if (!data) return toast(`${year} 年電子試卷尚未建置完成，請點「官方原始題本」查看官方 PDF`);
    launchAssessment({
      kind: "archive",
      id: `ARCHIVE-${year}`,
      year,
      title: `${year} 年國中教育會考自然科題本（理化部分）`,
  ...
  function renderAnalysis() {
    $("#analysisStats").innerHTML = [
      [examFacts.approxCount, "自然科約略題數"], [examFacts.minutes, "自然科作答分鐘"], [units.length, "本站理化單元"], [domains.length, "六大領域"], [window.EXAM_ENGINE.quizCatalog.length, "小考題庫份數"]
    ].map(([value, label]) => `<div><strong>${value}</strong><span>${label}</span></div>`).join("");

    $("#officialStructureTable").innerHTML = `
      <div class="chart-card">
        <h3>${esc(examFacts.subject)}</h3>
        <p>${esc(examFacts.format)}，近年約 ${examFacts.approxCount} 題、作答時間 ${examFacts.minutes} 分鐘。</p>
        <p>${esc(examFacts.note)}</p>
        <a class="secondary link-button" href="https://cap.rcpet.edu.tw/test4-5.html" target="_blank" rel="noopener">官方自然科考試內容 ↗</a>
      </div>`;

    const domainCounts = {};
    units.forEach(unit => { domainCounts[unit.domain] = (domainCounts[unit.domain] || 0) + 1; });
    const maxCount = Math.max(...Object.values(domainCounts));
    $("#domainDistribution").innerHTML = domains.map(d => `<article class="domain-stat"><span>${esc(d.name)}</span><strong>${domainCounts[d.name] || 0}</strong><small>單元數｜占本站理化範圍 ${(((domainCounts[d.name] || 0) / units.length) * 100).toFixed(0)}%</small></article>`).join("");
    $("#abilityBars").innerHTML = domains.map(d => `<div class="bar-row"><span>${esc(d.name)}</span><div><i style="--value:${(domainCounts[d.name] || 0) / maxCount * 100}%"></i></div><b>${domainCounts[d.name] || 0}</b></div>`).join("");

    $("#conceptMatrixHead").innerHTML = `<tr><th>單元</th><th>年級</th><th>學期</th><th>所屬領域</th></tr>`;
    $("#conceptMatrixBody").innerHTML = [8, 9].flatMap(grade => {
      const gradeUnits = units.filter(u => u.grade === grade);
      return gradeUnits.map((unit, i) => `<tr><td>${String(unit.id).padStart(2, "0")}｜${esc(unit.title)}</td><td>國${gradeName(grade)}</td><td>${i < gradeUnits.length / 2 + 0.5 ? "上／下依教學順序" : ""}</td><td>${esc(unit.domain)}</td></tr>`);
    }).join("");

    const bp = { total: units.length, domains: domains.map(d => `${d.name} ${domainCounts[d.name] || 0}`).join("、") };
    $("#blueprintChecks").innerHTML = [
      ["✓ 官方自然科結構", `${examFacts.format}，約 ${examFacts.approxCount} 題／${examFacts.minutes} 分鐘（生物＋理化＋地球科學）`],
  ...
  function quizCard(item) {
    ...
    return `<article class="quiz-card ${item.term === "總複習" ? "total" : ""} ${isChapter ? "chapter" : ""}">
      <div class="quiz-card-top"><span>${esc(isChapter ? `${item.book} ${item.chapter}` : item.term)}</span><small>${item.questionCount || 10} 題｜${item.minutes || 15} 分鐘</small></div>
      <h3>${esc(item.title)}</h3>
      <p>${isChapter ? "本單元獨立題庫；交卷後逐題顯示公式、詳解、技巧與易錯點。" : "四選一、即時計分、逐題詳解；題目只從此範圍生成。"}</p>
      <div class="quiz-unit-list">${scopeUnits.map(title => `<span>${esc(title)}</span>`).join("")}</div>
      <small class="quiz-official-code">所屬領域：${esc(item.domainNote)}</small>
  ...
  function renderQuizCatalog() {
    ...
    $("#quizCatalog").innerHTML = [8, 9].map(grade => {
      ...
      return `<section class="quiz-grade-section">
        ...
        <div class="quiz-track-grid">
          ${quizTrack("上學期", `${upper.length} 個理化單元小考`, upper, grade, "upper")}
          ${quizTrack("下學期", `${lower.length} 個理化單元小考`, lower, grade, "lower")}
  ...
  function renderMyPapers() {
    ...
    $("#paperHistoryStats").innerHTML = [
      ...
      ["模擬測驗", records.filter(item => item.kind === "mock").length],
    ...
    }).join("") : `<div class="paper-history-empty"><h2>目前還沒有考過的卷子。</h2><p>完成任一小考或模擬測驗後，這裡會自動保存紀錄。</p><button class="primary" data-view="quiz">去做小考</button></div>`;
  ...
  function configureExamHeader() {
    ...
    $("#examEyebrow").textContent = isQuiz ? "UNIT-SCOPE QUIZ" : isArchive ? "OFFICIAL PAST PAPER" : "ORIGINAL PRACTICE EXAM";
    $("#examTitle").textContent = isQuiz || isArchive ? state.exam.title : "理化模擬測驗";
    $("#examDescription").textContent = isQuiz
      ? `${state.exam.questions.length} 題四選一，共 ${state.exam.minutes || 15} 分鐘。所屬領域：${state.exam.officialCodes}。`
      : isArchive
      ? `依 ${state.exam.year} 年官方公布題本製作的可作答電子試卷；計時結束後仍可繼續作答。`
      : "涵蓋 21 個理化單元的原創練習題，全部為四選一，約 40 分鐘。不是官方自然科題本的節錄。";
    $("#examSetup").classList.toggle("hidden", isQuiz || isArchive);
    $("#quizExamSetup").classList.toggle("hidden", !isQuiz && !isArchive);
    if (isArchive) { $("#backToListButton").textContent = "返回歷屆考卷"; $("#backToListButton").dataset.view = "archive"; }
  ...
  function beginExam() {
    ...
    assessment.title = "理化模擬測驗";
    assessment.minutes = 40;
    launchAssessment(assessment);
    localStorage.setItem("capScience.lastSeed", seed);
  }
  ...
  function renderExam() {
    ...
    const cover = isQuiz ? `
      <header class="paper-cover"><div><p class="eyebrow">單元範圍 · ${esc(state.exam.id)}</p><h2>${esc(state.exam.title)}</h2><p>${state.exam.questions.length} 題四選一｜${state.exam.minutes || 15} 分鐘｜每個列出單元至少覆蓋 1 題</p></div><div class="paper-stamp">國${gradeName(state.exam.grade)}<br>${esc(state.exam.term)}</div></header>
      <div class="paper-instructions"><div><strong>${state.exam.questions.length}</strong><span>四選一｜即時計分</span></div><div><strong>${state.exam.unitIds.length}</strong><span>範圍單元｜無超綱單元</span></div><div><strong>${state.exam.minutes || 15} min</strong><span>依單元安排難度</span></div></div>
      <div class="quiz-paper-scope"><strong>本卷範圍</strong><span>${esc(scopeTitles)}</span><small>所屬領域：${esc(state.exam.officialCodes)}</small></div>` : isArchive ? `
      <header class="paper-cover"><div><p class="eyebrow">官方題本重現 · ${esc(state.exam.id)}</p><h2>${state.exam.year} 年國中教育會考自然科（理化部分）</h2><p>${mcCount} 題選擇｜依官方公布題目製作為可作答電子試卷｜計時結束仍可繼續作答</p></div><div class="paper-stamp">${state.exam.year}<br>官方題本</div></header>
      <div class="paper-instructions"><div><strong>${mcCount}</strong><span>四選一｜官方原題</span></div><div><strong>${state.exam.minutes || 60} min</strong><span>時間到可繼續作答</span></div></div>
      ${state.exam.omittedNote ? `<div class="quiz-paper-scope"><strong>收錄說明</strong><span>${esc(state.exam.omittedNote)}</span></div>` : ""}` : `
      <header class="paper-cover"><div><p class="eyebrow">21 單元原創練習 · ${esc(state.exam.id)}</p><h2>理化模擬測驗</h2><p>全部為四選一選擇題｜約 40 分鐘｜涵蓋 21 個理化單元</p></div><div class="paper-stamp">理化<br>範圍練習</div></header>
  ...
  function submitExam() {
    ...
    savePaperRecord({
      ...
      title: state.exam.title || (isQuiz ? "小考" : "模擬測驗"),
    ...
    const resultNote = isQuiz ? `本小考只計入「${esc(state.exam.title)}」的範圍，原始答對數不等同學校定期評量成績。`
      ...
      : "本結果是練習用原始答對數，不等同官方等級，也不代表官方自然科理化占分比例。";
  ...
  function bindStaticEvents() {
    ...
    $("#themeButton").addEventListener("click", () => {
      ...
      localStorage.setItem("capScience.dark", dark ? "1" : "0");
  ...
  function init() {
    const lastSeed = localStorage.getItem("capScience.lastSeed");
    ...
    if (localStorage.getItem("capScience.dark") === "1") { document.body.classList.add("dark"); $("#themeButton").textContent = "日"; }
    bindStaticEvents();
