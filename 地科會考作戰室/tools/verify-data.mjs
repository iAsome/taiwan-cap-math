...
for (const file of ["earth-data.js", "questions.js", "analysis-data.js"]) {
  const code = fs.readFileSync(path.join(root, file), "utf8");
  vm.runInContext(code, sandbox, { filename: file });
}

const { EARTH_DATA, EXAM_ENGINE, EARTH_ANALYSIS } = sandbox.window;
const errors = [];
...
const analysisObj = EARTH_ANALYSIS;
const analysisName = "EARTH_ANALYSIS";
...
if (errors.length) {
  ...
  const years = Object.keys(EARTH_ANALYSIS.primaryUnits).sort();
  const totalArchiveQ = years.reduce((s, y) => s + EARTH_ANALYSIS.primaryUnits[y].length, 0);
