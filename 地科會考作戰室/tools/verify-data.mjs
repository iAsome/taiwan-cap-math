// 自我檢查腳本：載入地科站的資料模組，檢查跨檔案的資料一致性。
// 用法：node tools/verify-data.mjs（在 地科會考作戰室/ 目錄下執行）
...
for (const file of ["earth-data.js", "questions.js", "analysis-data.js", "archive-exams.js"]) {
  const code = fs.readFileSync(path.join(root, file), "utf8");
  vm.runInContext(code, sandbox, { filename: file });
}

const { EARTH_DATA, EXAM_ENGINE, EARTH_ANALYSIS, ARCHIVE_EXAMS } = sandbox.window;
const errors = [];
const check = (cond, msg) => { if (!cond) errors.push(msg); };

// 1. 單元 id 應為 1..20 連續整數
const unitIds = EARTH_DATA.units.map(u => u.id);
check(unitIds.length === 20, `EARTH_DATA.units 應有 20 個單元，實際 ${unitIds.length} 個`);
...
for (const blueprint of EXAM_ENGINE.quizCatalog) {
  ...
  blueprint.unitIds.forEach(id => check(unitIds.includes(id), `小考 ${blueprint.id} 引用的 unitId=${id} 不存在於 EARTH_DATA`));
}

// 3. 考卷典藏：unitId 有效、四選項、answer 索引有效、steps/tip/trap 齊全
Object.entries(ARCHIVE_EXAMS).forEach(([year, exam]) => {
  exam.questions.forEach((q, i) => {
    check(unitIds.includes(q.unitId), `archive-exams ${year} 第 ${i + 1} 題 unitId=${q.unitId} 不存在於 EARTH_DATA`);
...
Object.entries(EARTH_ANALYSIS.primaryUnits).forEach(([year, unitSeq]) => {
  ...
  const info = EARTH_ANALYSIS.officialByYear[year];
...
Object.keys(ARCHIVE_EXAMS).forEach(year => check(!!EARTH_ANALYSIS.primaryUnits[year], `archive-exams 有 ${year} 年，但 analysis-data primaryUnits 沒有登記`));

// 5. domainByUnit 必須涵蓋所有單元 id
unitIds.forEach(id => check(!!EARTH_ANALYSIS.domainByUnit[id], `analysis-data domainByUnit 缺少單元 ${id}`));
