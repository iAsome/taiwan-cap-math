// 地科模擬考／小考出題引擎。
// 自然科會考全為四選一選擇題，沒有非選擇題，因此本引擎不需要 cr() helper。
...
window.EXAM_ENGINE = (() => {
  ...
  function withTextOnlyPolicy(question) {
    return typeof TEXT_ONLY_POLICY !== "undefined" ? TEXT_ONLY_POLICY.normalizeQuestion(question, "earth") : question;
  }
  ...
  const reviewQuizzes = window.EARTH_DATA.groups.map(g => ({
    id: `d${g.id}-all`, group: g.id, chapter: "總複習", title: `${groupNames[g.id]}．領域總複習`,
    unitIds: U.filter(u => u.grade === g.id).map(u => u.id), officialCodes: `EARTH-${groupNames[g.id]}`, scope: "review",
  ...
  function generateQuiz(quizId, seedOverride) {
    ...
    return {
      ...
      unitIds: [...blueprint.unitIds], blueprint: "NAER-108-curriculum-earth-science-scope", taxonomySource: blueprint.source, questions
    };
  }
  ...
  function generate(seed, level = 2) {
    ...
    // ponytail: 沒有官方獨立地科考科規格可比照，本站自訂 30 題／40 分鐘，涵蓋全部 20 單元
    // 並在板塊構造、天氣兩個歷屆自然科占比較高的領域各加抽 5 題，頁面上會清楚聲明此為自訂比例。
    const allUnitIds = U.map(u => u.id);
    const extraWeighted = [...U.filter(u => u.grade === 3).map(u => u.id), ...U.filter(u => u.grade === 5).map(u => u.id)];
    ...
    return { id: `EARTH-${seed}-${level}`, seed: Number(seed), level, createdAt: new Date().toISOString(), blueprint: "self-defined-30q-40min-earth-science-scope", questions };
  }
