(() => {
  ...
  const abilityLabel = { knowledge: "知識理解", comprehension: "統整判讀", inquiry: "科學探究" };
  const viewNames = { home: "學習總覽", exam: "全範圍模擬考", quiz: "單元小考題庫", papers: "我的考卷", handbook: "國中地科全冊講義", atlas: "題型與技巧地圖", analysis: "近十年逐題分析", sources: "資料與技巧審核", archive: "近十年考卷館" };
  let toastTimer;
  ...
  const state = {
    ...
    tipSearch: "",
    tipVerdict: "all",
    completed: new Set(JSON.parse(localStorage.getItem("capEarth.completed") || "[]")),
    exam: null,
    answers: [],
    submitted: false,
    seconds: 2400,
  ...
  function uniqueQuizAssessment(quizId) {
    const used = readJson(`capEarth.quizSignatures.${quizId}`, []);
    for (let attempt = 0; attempt < 80; attempt++) {
      ...
      if (!used.includes(signature)) {
        localStorage.setItem(`capEarth.quizSignatures.${quizId}`, JSON.stringify([signature, ...used].slice(0, 300)));
        return assessment;
      }
    }
  ...
  function paperHistory() { return readJson("capEarth.paperHistory", []); }

  function savePaperRecord(record) {
    try { localStorage.setItem("capEarth.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 40))); }
    catch {
      try { localStorage.setItem("capEarth.paperHistory", JSON.stringify([record, ...paperHistory()].slice(0, 15))); }
  ...
  function renderHandbook() {
    ...
    $("#unitContent").innerHTML = `
      <article class="unit-card">
        ...
        <div class="unit-body">
          ...
          <section class="lesson-block"><div class="lesson-label">關鍵數據與成因</div><div class="lesson-content"><div class="formula-box">${block(unit.formula)}</div><h3>為什麼會這樣</h3><p>${nl(unit.derivation)}</p></div></section>
    ...
    $("[data-complete]").addEventListener("click", () => {
      ...
      localStorage.setItem("capEarth.completed", JSON.stringify([...state.completed]));
      updateLearningProgress();
      renderHandbook();
  ...
  function renderAtlas() {
    $("#atlasContent").innerHTML = `
      ...
      <section class="strategy-section"><p class="eyebrow">PATTERN → TOOL</p><h2>常見地科題型：看到什麼，就啟動什麼判斷方式</h2>
  ...
  function renderArchive() {
    $("#archiveGrid").innerHTML = archives.map((item, index) => {
      ...
      <article class="archive-card ${index === 0 ? "latest" : ""}">
        ...
        <div class="archive-body"><h3>${item.year} 年國中教育會考自然科．地球科學題目</h3><p>${data ? `${count} 題選擇｜依官方自然科題本中的地科題目製作，可作答並看詳解` : "電子試卷建置中，敬請期待"}</p>
  ...
  function beginArchiveExam(year) {
    ...
    launchAssessment({
      ...
      id: `ARCHIVE-${year}`,
      year,
      title: `${year} 年國中教育會考自然科．地球科學題目`,
      minutes: data.minutes || 20,
      officialCodes: `official-${year}-earth`,
  ...
  function renderAnalysis() {
    ...
    $("#analysisStats").innerHTML = [
      ["10", "屆官方自然科題本"], [total, "逐題完成主概念編碼"], [Math.round(total / years.length * 10) / 10, "平均每屆地科題數"]
    ...
    $("#officialStructureTable").innerHTML = `<table class="mini-table"><thead><tr><th>年度</th><th>地科題數</th><th>知識理解</th><th>統整判讀</th><th>科學探究</th></tr></thead><tbody>${years.map(year => `<tr><td>${year}</td><td>${official[year].count}</td><td>${official[year].abilities.knowledge}</td><td>${official[year].abilities.comprehension}</td><td>${official[year].abilities.inquiry}</td></tr>`).join("")}</tbody></table>`;
    ...
    $("#yearLedger").innerHTML = years.slice().reverse().map((year, index) => {
      ...
      return `<details ${index === 0 ? "open" : ""}><summary><strong>${year}</strong><span>${info.count} 題地科｜自然科全卷 20 分鐘配比估算</span><b>展開逐題編碼 ＋</b></summary><div class="ledger-body"><div class="ledger-ability"><span>知識理解 ${info.abilities.knowledge}</span><span>統整判讀 ${info.abilities.comprehension}</span><span>科學探究 ${info.abilities.inquiry}</span></div><div class="ledger-questions">${primary[year].map((unitId, i) => `<article class="ledger-item"><b>${i + 1}</b><span>${esc(units.find(u => u.id === unitId)?.title || "跨單元")}<small>${esc(capAnalysis.domainByUnit[unitId])}</small></span></article>`).join("")}</div></div></details>`;
    }).join("");
  }
  ...
  function capSummary(item, compact = false) {
    ...
    return `<div class="quiz-cap-tags"><strong>會考標註 ${items.length} 題</strong><small>106–115 年自然科地球科學主概念回查</small><div class="quiz-cap-preview">${preview || "<span>近十年未列為主概念；仍依課綱出題</span>"}</div></div>`;
  }
  ...
  function configureExamHeader() {
    ...
    $("#examTitle").textContent = isQuiz || isArchive ? state.exam.title : "會考地科模擬考";
    $("#examDescription").textContent = isQuiz
      ...
      : isArchive
      ? `依 ${state.exam.year} 年官方自然科題本中的地球科學題目製作的可作答電子試卷；計時結束後仍可繼續作答。`
      : "30 題四選一，共 40 分鐘。涵蓋 20 個地科單元，板塊構造與天氣兩大領域加抽比重；本規格為本站自訂比例，非官方獨立考科規格。";
  ...
  function beginExam() {
    ...
    assessment.title = "會考地科模擬考";
    assessment.minutes = 40;
    launchAssessment(assessment);
    localStorage.setItem("capEarth.lastSeed", seed);
  }
  ...
  function renderExam() {
    ...
    const cover = isQuiz ? `
      ...
      <header class="paper-cover"><div><p class="eyebrow">官方題本重現 · ${esc(state.exam.id)}</p><h2>${state.exam.year} 年國中教育會考自然科．地球科學題目</h2><p>${state.exam.questions.length} 題選擇｜依官方公布題目中屬於地球科學的題號製作為可作答電子試卷｜計時結束仍可繼續作答</p></div><div class="paper-stamp">${state.exam.year}<br>官方題本</div></header>
      ...
      <header class="paper-cover"><div><p class="eyebrow">自訂比例 · 涵蓋 20 單元 · ${esc(state.exam.id)}</p><h2>會考地科模擬考</h2><p>30 題選擇｜40 分鐘｜板塊構造與天氣兩大領域加抽比重（本站自訂，非官方獨立考科規格）</p></div><div class="paper-stamp">地科<br>自訂規格</div></header>
      <div class="paper-instructions"><div><strong>30</strong><span>四選一｜全數地科單元覆蓋</span></div><div><strong>40 min</strong><span>題目順序依卷別種子打亂</span></div></div>`;
    $("#paper").innerHTML = `
      ${cover}
  ...
  function submitExam() {
    ...
    const resultNote = isQuiz ? `本小考只計入「${esc(state.exam.title)}」的範圍，原始答對數不等同學校定期評量成績。`
      : isArchive ? `本卷依 ${state.exam.year} 年官方公布題本中的地球科學題目重製，供練習使用；正式成績、等級與官方原始題本以會考官方網站為準。`
      : "本結果是練習用原始答對數，不等同官方等級；本模考規格為本站自訂，非官方獨立地科考科規格。";
  ...
  function bindStaticEvents() {
    ...
    $("#themeButton").addEventListener("click", () => {
      ...
      localStorage.setItem("capEarth.dark", dark ? "1" : "0");
  ...
  function init() {
    const lastSeed = localStorage.getItem("capEarth.lastSeed");
    ...
    if (localStorage.getItem("capEarth.dark") === "1") { document.body.classList.add("dark"); $("#themeButton").textContent = "日"; }
    bindStaticEvents();
