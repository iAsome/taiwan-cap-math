window.DIAGRAM_INFER = (() => {
  ...
  function inferTriangle(text, topicTitle) {
    ...
    const angleLabels = [];
    const ang = firstNum(raw, /[∠∟]?([A-Z]{1,3})\s*[=\＝]\s*(\d+)\s*°/);
    if (ang) angleLabels.push({ at: ang, label: `${ang}°` });
  ...
  function inferMath(text, ctx) {
    ...
    if (/長條|折線|圖表|統計/.test(blob)) return inferBarChart(text, topic);
  ...
  function inferGeography(text, ctx) {
    ...
    if (/長條|折線|統計|圖表/.test(blob)) return inferBarChart(text, ctx.topicTitle);
  ...
  function validateSpecLabels(text, spec) {
    if (!spec) return true;
    const raw = stripFrac(text);
    if (spec.kind === "circle" && /半徑\s*\d+/.test(raw)) {
      const n = firstNum(raw, /半徑[為是]?\s*(\d+)/);
